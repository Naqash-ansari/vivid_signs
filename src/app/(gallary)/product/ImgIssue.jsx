import img from '../../assets/productList/bg_C.png'
...
const ImgIssue = () => {
    ...
    return (
        <div className="relative h-full flex justify-center items-center">
            ...
            {/* Text Overlay */}
            <div className="absolute h-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
                <div className='text-center py-16 relative'>
                    ...
                    <main className="main bd-grid py-6 z-50">
                        ...
                        {currentProducts.map((product) => (
                            ...
                            <article className="card shadow-2xl" key={product.id}>
                    ...
                    <div className="row flex text-center justify-center ">
                        {/* Prev button */}
                        <button
                            className="rounded-md mx-1 rounded-r-none border border-r-0 border-slate-300 py-2 px-3 text-center text-sm transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-slate-800 hover:border-slate-800 focus:text-white focus:bg-slate-800 focus:border-slate-800 active:border-slate-800 active:text-white active:bg-slate-800 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                            type="button"
                        ...
                        {[...Array(totalPages).keys()].map((page) => (
                            <button
                                ...
                                className={`px-4 py-2 mx-1 ${currentPage === page + 1 ? 'bg-slate-800' : 'bg-gray-300'} text-white rounded`}
                                type="button"
                        ...
                        {/* Next button */}
                        <button
                            className="rounded-md mx-1 rounded-l-none border border-slate-300 py-2 px-3 text-center text-sm transition-all shadow-sm hover:shadow-lg text-slate-600 hover:text-white hover:bg-slate-800 hover:border-slate-800 focus:text-white focus:bg-slate-800 focus:border-slate-800 active:border-slate-800 active:text-white active:bg-slate-800 disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none"
                            type="button"
